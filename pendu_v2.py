...
while vie != 0:
    ...
    while bool :
        ...
        for i in range(len(lettre_utilisé)):  
            if lettre == lettre_utilisé[i] or len(lettre)> 1: #demande de choisir une autre lettre si la lettre est deja utilisé 
                                                              #ou si la lettre ne fait pas 1 caractère 
                clear()
                print("vie restante: {0}".format(vie))
                print("lettre utilisé: {0}".format(" , ".join(lettre_utilisé)))
                print("mot caché : {0}".format("".join(mot_caché)))
